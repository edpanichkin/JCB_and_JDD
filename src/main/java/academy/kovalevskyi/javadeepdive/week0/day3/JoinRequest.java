...
public class JoinRequest extends AbstractRequest<Csv> {
  ...
  protected Csv execute() throws RequestException {
    ...
    int indexByOn = indexOfValue(csv.headers(), byValue);
    int indexByFrom = indexOfValue(on.headers(), byValue);
    ...
    for (int i = 0; i < newValues.length; i++) {
      if (csv.values()[i][indexByOn].equals(on.values()[i][indexByFrom])) {
        System.arraycopy(csv.values()[count], 0, newValues[count], 0, csv.values()[count].length);
        System.arraycopy(on.values()[count], 1, newValues[count], csv.values()[count].length,
            on.values()[count].length - 1);
        count++;
      }
    }
    return new Csv.Builder().header(newHeaders).values(newValues).build();
  }
